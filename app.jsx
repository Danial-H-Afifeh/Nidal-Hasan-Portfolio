...
const logoUrl = `${ASSET_BASE}logo.png`;
const avatarUrl = `${ASSET_BASE}avatar.png`;
...
const updates = [
  {
    type: "تنبيه مهم",
    date: "يُحدّث دورياً",
    title: "مراجعة التعرفة والرسوم قبل وصول الشحنة",
    text: "تتغير التعليمات والرسوم بحسب نوع البضاعة وقرارات الجهات المختصة. نراجع معكم الملف قبل المباشرة.",
  },
  {
    type: "إرشاد للمستوردين",
    date: "دليل عملي",
    title: "اكتمال المستندات يختصر وقت المعاملة",
    text: "جهّز بوليصة الشحن والفاتورة التجارية وقائمة التعبئة وأي موافقات مطلوبة قبل وصول الشحنة.",
  },
  {
    type: "من المكتب",
    date: "تحديث الخدمة",
    title: "  متابعة معاملات طرطوس واللاذقيةومعبر باب الهوا",
    text: "ننسّق مع شبكة مندوبي المكتب لتقديم متابعة أقرب للشحنات في المرافئ والمعابر التي نخدمها.",
  },
];
...
function AdminPanel({ onClose, onWorksChanged, works, onAuthenticated }) {
  ...
  const loginAdmin = async (event) => {
    ...
    if (!hasSupabase) {
      if (!LOCAL_ADMIN_PASSWORD) { setMessage('لم يتم إعداد خدمة الدخول. أضف VITE_SUPABASE_URL و VITE_SUPABASE_ANON_KEY إلى إعدادات بناء الموقع.'); return; }
      if (login.password !== LOCAL_ADMIN_PASSWORD) { setMessage('كلمة المرور المحلية غير صحيحة.'); return; }
      const localSession = { local: true }; setSession(localSession); onAuthenticated(localSession); setMessage('تم الدخول بالوضع المحلي المجاني.'); return;
    }
    try {
      ...
      setSession(result); onAuthenticated(result); setMessage('تم تسجيل الدخول بنجاح.');
...
function App() {
  ...
  return <div className="app"><div className="cursor-glow" /><Navbar /><main><Hero /><Services /><WorkLog works={works} isAdmin={Boolean(adminSession)} onAdmin={() => setAdminOpen(true)} onShowAll={() => setArchiveOpen(true)} /><Updates /><Documents /><Contact /></main><Footer onAdmin={() => setAdminOpen(true)} />{adminOpen && <AdminPanel works={works} onAuthenticated={setAdminSession} onClose={() => setAdminOpen(false)} onWorksChanged={loadWorks} />}{archiveOpen && <WorkArchive works={works} onClose={() => setArchiveOpen(false)} />}</div>;
}
